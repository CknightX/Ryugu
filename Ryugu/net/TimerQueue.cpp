...
namespace ryugu
{
	namespace net
	{
		...
		// 从now到when的时间
		timespec timeFromNow(base::Timestamp when)
		{
			int64_t microseconds = when.getTimestampByMicroSeconds() - base::Timestamp::getNow().getTimestampByMicroSeconds();
			// 小于100ms按100ms
			microseconds = microseconds < 100 ? 100 : microseconds;

			timespec ts;
			ts.tv_sec = static_cast<time_t> (microseconds / base::Timestamp::cstMicroSecondsPerSecond);
			ts.tv_nsec = static_cast<long>(
				(microseconds % base::Timestamp::cstMicroSecondsPerSecond) * 1000
				);
			return ts;
		}
		...
		void TimerQueue::resetTimerfd(int timerfd, base::Timestamp expiration)
		{
		...
		void TimerQueue::handleRead()
		{
			...
			base::Timestamp now(base::Timestamp::getNow());
			// 水平触发，需要读出，防止反复触发
		...
		bool TimerQueue::insert(Timer* timer)
		{
			...
			base::Timestamp when = timer->getExpiration();
